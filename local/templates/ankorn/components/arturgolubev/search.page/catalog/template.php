...
<div class="bx-ag-search-page search-page <?=$arResult["VISUAL_PARAMS"]["THEME_CLASS"]?>">
	...
	<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
		...
		<div class="search-language-guess">
			<?echo GetMessage("CT_BSP_KEYBOARD_WARNING", array("#query#"=>'<a href="'.$arResult["ORIGINAL_QUERY_URL"].'">'.htmlspecialcharsbx($arResult["REQUEST"]["ORIGINAL_QUERY"]).'</a>'))?>
		</div><br /><?
	endif;?>
	...
	<?if(is_array($arResult["SEARCH_HISTORY"]) && count($arResult["SEARCH_HISTORY"]) > 0):?>
		<div class="ag-spage-history">
			...
			<?foreach($arResult["SEARCH_HISTORY"] as $k=>$v):
				if($k > 0) echo ', ';?><a href="?q=<?=htmlspecialcharsbx($v)?>"><?=htmlspecialcharsbx($v)?></a><?
			endforeach?>
		</div>
	<?endif;?>
	...
	<?if(is_array($arResult["CLARIFY_SECTION"]) && count($arResult["CLARIFY_SECTION"]) > 1):?>
		<div class="ag-spage-clarify-list">
			...
			<?foreach($arResult["CLARIFY_SECTION"] as $section):?>
				<a class="ag-spage-clarify-item <?if($arResult["SELECTED_SECTION"] == $section["ID"]) echo 'selected';?>" href="<?=$APPLICATION->GetCurPageParam('section='.intval($section["ID"]), array("section"))?>"><?=htmlspecialcharsbx($section["NAME"])?> (<?=intval($section["CNT"])?>)</a>
			<?endforeach;?>
		</div>
	<?endif;?>
	...
	<?/* if(count($arResult["SEARCH"])>0):?>
		<div class="search-view-default">
			<b><?=GetMessage("AG_SPAGE_FINDED_SECTION")?>:</b>
