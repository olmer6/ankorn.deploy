...
else
{
	...
	?>
	<div class="page-container page-container-oder">
		<h1 id="basket-h1">Оформление заказа</h1>
		<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" class="bx-soa-wrapper mb-4<?=$themeClass?>" id="bx-soa-order-form" enctype="multipart/form-data">
			...
			<div id="bx-soa-order" class="row" style="opacity: 0">
				...
				<div class="col-lg-8 col-md-7 bx-soa">
					...
					<div class="bx-soa-cart-custom">После нажатия кнопки «Оформить заказ» наш менеджер свяжется с вами в течение 15 минут для уточнения деталей. Это необходимо, чтобы оборудование полностью соответствовало вашим задачам и было подобрано без ошибок. Счет на оплату выставляется после подтверждения параметров, день в день.</div>
