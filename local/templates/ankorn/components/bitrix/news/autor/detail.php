...
AddEventHandler("main", "OnEndBufferContent", "modifyCanonicalSlash");
function modifyCanonicalSlash(&$content) {
    $pattern = '/<link\s+rel="canonical"\s+href="([^"]*?)"(\s*\/?)>/i';
    $content = preg_replace_callback($pattern, function($matches) {
        $url = $matches[1];
        // Добавляем слэш, если его нет и URL не содержит параметров
        if (!strpos($url, '?') && substr($url, -1) !== '/') {
            $url .= '/';
        }
        return '<link rel="canonical" href="' . $url . '"' . $matches[2] . '>';
    }, $content);
}
