...
<div class="row bx-<?=$arParams['TEMPLATE_THEME']?>">
	<div class='<?=($isSidebar ? 'col-md-9 col-sm-8' : 'col')?>'>
		...
		$elementId = $APPLICATION->IncludeComponent('bitrix:catalog.element', 'ankorn', $componentElementParams,
			$component
		);
		...
		if ($elementId > 0)
		{
			...
			if (!empty($recommendedData))
			{
				...
				$arParams['DETAIL_SHOW_POPULAR'] = 'N'; /* Принудительно отключаем показ популярных */
				...
				$arParams['DETAIL_SHOW_VIEWED'] = 'N'; // Принудетельно отключаем просмотренные
				if (
