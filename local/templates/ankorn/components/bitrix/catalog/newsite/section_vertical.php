...
?>
<?php
print "<div class='page-container page-container-top page-container-catpage'>";
print "<div id='catalog-breadcrumb'>";
...
<div class="row mb-4 bx-<?=$arParams["TEMPLATE_THEME"]?> catalog_section_wrap">
		...
		<div class="br-grey col-lg-3 col-md-4 col-sm-5<?=(isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' d-none d-sm-block' : '')?>">
			...
			if ($isFilter): ?>
				<div class="bx-sidebar-block">
					<?
					$APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", "ankorn", array(
	...
	<div class="catalog-body pb-4 <?=(($isFilter || $isSidebar) ? "col-lg-9 col-md-8 col-sm-7" : "col")?>">
		<?
		if (ModuleManager::isModuleInstalled("sale"))
		{
			...
			if (!empty($arRecomData) && $arParams['USE_GIFTS_SECTION'] === 'Y')
			{
				...
				<div class="row">
					...
					<?php
					// Краткое описание и пользовательские ссылки
						if (!empty($arCurSection['ID'])) {
						    $connection = \Bitrix\Main\Application::getConnection();
						  	// Краткое описание
						    $sql = "SELECT UF_DESCSHORT FROM b_uts_iblock_2_section WHERE VALUE_ID = " . (int)$arCurSection['ID'];
						    $result = $connection->query($sql);
						    if ($row = $result->fetch()) {
						        if (!empty($row['UF_DESCSHORT'])) {
						            echo '<div class="short-desc">' . htmlspecialcharsbx($row['UF_DESCSHORT']) . '</div>';
						        }
						    }
						    // Пользовательские ссылки

						    // 1. Получаем сериализованное значение поля UF_LINKS
						    $sql = "SELECT UF_LINKS 
						            FROM b_uts_iblock_2_section 
						            WHERE VALUE_ID = " . $arCurSection['ID'];
						    
						    $result = $connection->query($sql);
						    $row = $result->fetch();
						    					    
						    $serializedData = $row['UF_LINKS'];

						    // 2. Десериализуем данные
						    $linksArray = unserialize($serializedData);

						 // 3. Проверяем и выводим результат
						    if (is_array($linksArray) && !empty($linksArray)) {
						        ...
						        foreach ($linksArray as $linkItem) {
						            ...
						            // Разделяем "URL=Название"
						            $parts = explode('=', $linkItem, 2);
						            $url = isset($parts[0]) ? trim($parts[0]) : '#';
						            $name = isset($parts[1]) ? trim($parts[1]) : $url;
						            ...
						            $safeName = htmlspecialcharsbx($name);
						            
						            echo '<li><a href="' . $safeUrl . '">' . $safeName . '</a></li>';
						        }
...
/*			 закомментировано 29,03,2026 тут предыдущий пытался какой то ерунды с сортировкой понаделать
		...
		// 2. Создаем кастомное поле для приоритета наличия
		// Если у товара есть PROPERTY_PARAM_AVAILABLE_VALUE = 'available_yes' - ставим 1, иначе 0
		//$prioritySortField = "CASE WHEN (PROPERTY_PARAM_AVAILABLE_VALUE = 'В наличии') THEN 1 ELSE 0 END";

		//$prioritySortField = "PROPERTY_PARAM_AVAILABLE"; 
		$prioritySortField = "PROPERTY_S_PRIORITY"; 

		// $prioritySortField = "CASE 
		//     WHEN (PROPERTY_ISMAIN_VALUE IS NOT NULL AND PROPERTY_PARAM_AVAILABLE_VALUE = 'В наличии') THEN 3
		//     WHEN (PROPERTY_ISMAIN_VALUE IS NOT NULL) THEN 2
		//     WHEN (PROPERTY_PARAM_AVAILABLE_VALUE = 'В наличии') THEN 1
		//     ELSE 0
		// END";

		//$prioritySortOrder = "DESC"; // "В наличии" будет выше, чем пустое значение

		// 3. Переопределяем параметры сортировки
		$newSortParams = array(
		    //"ELEMENT_SORT_FIELD" => "SORT_PRIORITY",  // ПЕРВЫЙ уровень: приоритет наличия
				"ELEMENT_SORT_FIELD" => $prioritySortField,  
		    "ELEMENT_SORT_ORDER" => "DESC",              // DESC = 1 сверху (в наличии), 0 снизу (нет в наличии)
		    "ELEMENT_SORT_FIELD2" => $userSortField,     // ВТОРОЙ уровень: то, что выбрал пользователь
		    "ELEMENT_SORT_ORDER2" => $userSortOrder,
...
*/
		...
		$componentParams = array(
		    "IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
		    "IBLOCK_ID" => $arParams["IBLOCK_ID"],
		    // ... все ваши текущие параметры из array() ...
				"PROPERTY_CODE" => (isset($arParams["LIST_PROPERTY_CODE"]) ? $arParams["LIST_PROPERTY_CODE"] : []),
				"PROPERTY_CODE_MOBILE" => $arParams["LIST_PROPERTY_CODE_MOBILE"],
				...
				if (!isset($arParams['USE_BIG_DATA']) || $arParams['USE_BIG_DATA'] != 'N')
				{
					...
					<div class="row mb-3">
						<div class="col" data-entity="parent-container">
							...
							<? $APPLICATION->IncludeComponent("bitrix:catalog.section", "ankorn", array(
									"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
									"IBLOCK_ID" => $arParams["IBLOCK_ID"],
									"ELEMENT_SORT_FIELD" => $arParams["ELEMENT_SORT_FIELD"],
									"ELEMENT_SORT_ORDER" => $arParams["ELEMENT_SORT_ORDER"],
